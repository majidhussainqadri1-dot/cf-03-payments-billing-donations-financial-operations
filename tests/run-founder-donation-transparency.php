...
$tests['donation creates no ownership right']=static function():void{$p=new FounderOwnershipPolicy();foreach(array_keys($p->donorRights())as$r){$p->assertNoDonorRight($r);sameFT(false,$p->donorRights()[$r]);}};
...
$tests['donation management remains fail closed without WordPress runtime']=static function():void{$r=WordPressRestApi::donationManagement();sameFT('sabri_cf03_internal_error',$r['code']);sameFT(500,$r['status']);};
...
$tests['transparency extension declares three tables']=static function():void{$t=TransparencySchema::tables('wp_');sameFT(['expenses','transparency_snapshots','donor_acknowledgments'],array_keys($t));};
$tests['complete schema version and table count are final']=static function():void{sameFT('3.3.0',CompleteSchema::VERSION);sameFT(31,count(CompleteSchema::tables('wp_')));};
$tests['complete schema includes founder expense disclosure fields']=static function():void{$s=CompleteSchema::tables('wp_')['expenses'];sameFT(true,str_contains($s,'founder_related'));sameFT(true,str_contains($s,'public_disclosure_category'));};
$tests['complete schema includes privacy consent acknowledgment']=static function():void{$s=CompleteSchema::tables('wp_')['donor_acknowledgments'];sameFT(true,str_contains($s,'consent_hash'));sameFT(true,str_contains($s,'revoked_at'));};
$tests['complete schema enforces audit serialization and settlement duties']=static function():void{$t=CompleteSchema::tables('wp_');sameFT(true,str_contains($t['audit'],'UNIQUE KEY previous_hash(previous_hash)'));sameFT(true,str_contains($t['settlements'],'imported_by varchar(191) NOT NULL'));};
...
function snapshotFT(DateTimeImmutable$n):FinancialTransparencySnapshot{return new FinancialTransparencySnapshot('snapshot:2026-08',$n,new Money(10000,'USD'),new Money(1000,'USD'),new Money(5000,'USD'),[DonationExpenseCategory::INSTITUTIONAL_OPERATIONS=>new Money(1500,'USD'),DonationExpenseCategory::TECHNICAL_DEVELOPMENT=>new Money(1000,'USD'),DonationExpenseCategory::HOMEOPATHY_ADVANCEMENT=>new Money(500,'USD'),DonationExpenseCategory::ADMINISTRATION_PAYMENT_CHARGES=>new Money(500,'USD'),DonationExpenseCategory::FOUNDER_COMPENSATION=>new Money(400,'USD'),DonationExpenseCategory::FOUNDER_EXPENSE_REIMBURSEMENT=>new Money(300,'USD'),DonationExpenseCategory::FOUNDER_ADVANCE_REPAYMENT=>new Money(200,'USD'),DonationExpenseCategory::OWNER_WITHDRAWAL=>new Money(100,'USD')],$n,str_repeat('a',64));}
function sameFT(mixed$e,mixed$a):void{if($e!==$a){throw new RuntimeException('Expected '.var_export($e,true).', got '.var_export($a,true));}}
/** @param class-string<Throwable> $class */function throwsFT(callable$c,string$class):void{try{$c();}catch(Throwable$e){if($e instanceof$class){return;}throw new RuntimeException('Expected '.$class.', got '.$e::class.': '.$e->getMessage());}throw new RuntimeException('Expected '.$class);}
