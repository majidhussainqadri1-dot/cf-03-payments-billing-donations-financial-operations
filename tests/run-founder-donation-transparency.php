...
$tests['snapshot calculates totals balance and founder amount']=static function()use($now):void{$s=snapshotFT($now);sameFT(4000,$s->totalExpenses()->minorUnits());sameFT(6000,$s->currentBalance()->minorUnits());sameFT(1000,$s->founderPaidOrWithdrawn()->minorUnits());};
$tests['snapshot public projection contains required aggregates']=static function()use($now):void{$p=snapshotFT($now)->toPublicProjection();sameFT(10000,$p['total_donations_minor']);sameFT(6000,$p['current_balance_minor']);sameFT(1000,$p['founder_paid_or_withdrawn_minor']);sameFT(false,$p['donor_identity_public']);};
