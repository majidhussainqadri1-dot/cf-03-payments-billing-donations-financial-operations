...
$add('review 09 settlement window', static function () use ($now): void { $i=readyIntent40($now); $e=new ProviderEvidence('provider.sandbox','event:late:1','payment.settled','intent:1',new Money(1400,'USD'),'key:v1',$now,$now,str_repeat('b',64),true,true,$now->modify('+2 hours')); throws40(static fn()=>settleService40()->confirmSettled($i,$e,ledger40(1400),3,$now->modify('+2 hours'),'event:settled:late'),DomainException::class); });
