...
$tests['01 schema 3.2.0 keeps 31 canonical tables']=static function():void{sameSC('3.2.0',CompleteSchema::VERSION);sameSC(31,count(CompleteSchema::tables('wp_')));};
...
/** @param list<array<string,mixed>> $entries @return array{0:int,1:int} */function totalsSC(array $entries):array{$debit=0;$credit=0;foreach($entries as $entry){if(($entry['direction']??null)==='debit'){$debit+=(int)$entry['amount_minor'];}elseif(($entry['direction']??null)==='credit'){$credit+=(int)$entry['amount_minor'];}}return[$debit,$credit];}
...
/** @param class-string<Throwable> $class */function throwsSC(callable $callback,string $class):void{try{$callback();}catch(Throwable $e){if($e instanceof $class){return;}throw new RuntimeException('Expected '.$class.', got '.$e::class.': '.$e->getMessage());}throw new RuntimeException('Expected '.$class);}
