...
final class ProviderWebhookVerifier
{
    ...
    /**
     * Expected raw JSON fields: event_id, type, intent_id, amount_minor, currency.
     */
    public function verify(
        ...
        foreach (['event_id', 'type', 'intent_id', 'amount_minor', 'currency'] as $field) {
        ...
        $evidence = new ProviderEvidence(
            ...
            new Money($payload['amount_minor'], strtoupper($payload['currency'])),
            $keyVersion,
            $signatureTimestamp,
            $receivedAt,
            hash('sha256', $rawBody),
