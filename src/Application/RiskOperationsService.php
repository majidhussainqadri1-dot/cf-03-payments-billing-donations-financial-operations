...
final class RiskOperationsService
{
    ...
    public function adjustChargebackLedger(string $caseId, string $operatorReference, DateTimeImmutable $at, int $expectedVersion): array
    {
        ...
        $financialDeltaExists = $case->state() === 'lost' || $fee->minorUnits() > 0;
        $transactionId = $financialDeltaExists
        ...
        $this->repository->transaction(function () use (
            ...
            if ($transactionId !== null && $this->repository->get('ledger_transactions', $transactionId) === null) {
        ...
        return [
            ...
            'transaction_id' => $transactionId,
            'no_financial_delta' => $transactionId === null,
    ...
    private function fraud(array $r): FraudReviewCase
    {
        return new FraudReviewCase(
            (string)$r['review_id'],
            (string)$r['subject_ref'],
            (array)$r['signals_json'],
            new DateTimeImmutable((string)$r['created_at']),
            new DateTimeImmutable((string)$r['hold_until']),
            (string)$r['state'],
            (int)$r['version'],
            is_string($r['reviewer_ref'] ?? null) ? $r['reviewer_ref'] : null,
            is_string($r['decision_reason'] ?? null) ? $r['decision_reason'] : null
        );
    }

    private function chargeback(array $r): ChargebackCase
    {
        return new ChargebackCase(
            (string)$r['case_id'],
            (string)$r['provider'],
            (string)$r['provider_case_ref'],
            (string)$r['intent_id'],
            new Money((int)$r['amount_minor'], (string)$r['currency']),
            (string)$r['reason_code'],
            new DateTimeImmutable((string)$r['created_at']),
            new DateTimeImmutable((string)$r['response_deadline']),
            (string)$r['state'],
            (int)$r['version'],
            is_string($r['evidence_hash'] ?? null) ? $r['evidence_hash'] : null,
            isset($r['provider_fee_minor']) && $r['provider_fee_minor'] !== null
                ? new Money((int)$r['provider_fee_minor'], (string)$r['currency'])
                : null
        );
    }
    ...
    {
        return $this->repository->compareAndSwap('fraud_reviews', $id, (int)$record['version'], static function (array $current) use ($case, $at): array {
    ...
    {
        return $this->repository->compareAndSwap('chargebacks', $id, (int)$record['version'], static function (array $current) use ($case, $at): array {
            $current['state'] = $case->state();
