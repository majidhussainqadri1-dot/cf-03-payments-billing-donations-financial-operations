...
final class DonationCheckoutService
{
    ...
    public function create(DonationIntentDraft $draft, string $providerCode): array
    {
        ...
        $claimId = 'idem.'.substr(hash('sha256', 'donation-checkout|'.$draft->donorReference().'|'.$draft->idempotencyKey()), 0, 48);
        ...
        try {
            $checkout = $this->providers->get($providerCode)->createHostedDonationCheckout($draft);
            ...
            $this->repository->transaction(function () use ($draft, $providerCode, $checkout, $requestHash, $claimId, $donationId, $consentId, $traceId, $now): void {
                ...
                if ($consentId !== null) {
                    $this->repository->insert('recurring_consents', $consentId, [
                        ...
                        'terms_hash' => hash('sha256', 'donation.monthly|'.$draft->amount()->minorUnits().'|'.$draft->amount()->currency()),
                ...
                $updated = $this->repository->updateWhere('idempotency', ['idempotency_key' => $claimId, 'state' => 'pending'], [
                    'state' => 'completed', 'result_ref' => $draft->intentId(), 'completed_at' => $now,
                ]);
                if ($updated !== 1) { throw new InvariantViolation('Donation checkout idempotency claim was not completed atomically.'); }
            });
            ...
            $this->repository->updateWhere('idempotency', ['idempotency_key' => $claimId, 'state' => 'pending'], [
                'state' => 'failed', 'completed_at' => new DateTimeImmutable('now'),
            ]);
            throw $error;
        }
    }
    ...
    /** @return array<string,mixed> */
    private function replay(array $claim, string $requestHash): array
    {
        ...
        if (($claim['state'] ?? null) !== 'completed' || !is_string($claim['result_ref'] ?? null)) {
            throw new InvariantViolation('Donation checkout request is already in progress or previously failed.');
        }
        $intent = $this->repository->get('intents', $claim['result_ref']);
        if ($intent === null) { throw new InvariantViolation('Completed donation idempotency record has no canonical intent.'); }
        return [
            'status' => (string)($intent['state'] ?? 'provider_pending'),
            'intent_id' => (string)$intent['intent_id'],
            'provider' => (string)$intent['provider'],
            'provider_session_reference' => $intent['provider_ref'] ?? null,
            'hosted_url' => null,
            'expires_at' => self::dateString($intent['expires_at'] ?? null),
