...
final class DonationRequestVelocityGuard
{
    ...
    public function assertAllowed(string $actorReference, DateTimeImmutable $now): void
    {
        ...
        $recentAttempts = 0;
        foreach ($this->repository->find('idempotency', ['actor_ref' => $actorReference], 50) as $claim) {
        ...
        $active = 0;
        foreach ($this->repository->find('intents', ['actor_ref' => $actorReference], 20) as $intent) {
