...
final class Plugin
{
    ...
    {
        if(!function_exists('add_option')||!function_exists('update_option')){throw new RuntimeException('WordPress option APIs are unavailable during CF-03 activation.');}
        add_option(self::OPTION_ACTIVATION_RECORD,[],'',false);update_option(self::OPTION_RUNTIME_STATUS,'schema_installing_fail_closed',false);update_option(self::OPTION_FINANCIAL_POLICY_DECISION,PlatformFinancialPolicy::DECISION_ID,false);
    ...
    {
        if(function_exists('add_action')){add_action('init',[self::class,'registerDonationPromptMeta']);add_action('rest_api_init',[WordPressRestApi::class,'register']);add_action('admin_notices',[self::class,'renderConditionalNotice']);}
    ...
    public static function renderConditionalNotice(): void
    {
        ...
        $message='CF-03 policy '.PlatformFinancialPolicy::DECISION_ID.': the founder-owned platform is not a trust; fixed fees are prohibited, commission is 0%, donations are voluntary, and public aggregate transparency is mandatory. Source and schema are installed, but live collection remains fail closed pending provider and external acceptance. '.($status->approved()?'Activation evidence is configured, but no live provider route exists.':'Missing activation gates: '.implode(', ',$status->missingGates()).'.');
        echo '<div class="notice notice-info"><p>'.esc_html($message).'</p></div>';
    }

    /** @param array<string,mixed> $tests @return array<string,mixed> */
    public static function registerSiteHealthTest(array $tests): array{$tests['direct']['sabri_cf03_activation_gate']=['label'=>'CF-03 Founder donation and transparency policy','test'=>[self::class,'runSiteHealthTest']];return $tests;}

    /** @return array<string,mixed> */
    public static function runSiteHealthTest(): array
    {
        $status=EvidenceBoundActivationGate::forWordPress()->evaluate();
        return ['label'=>'CF-03 policy source complete; collection remains fail closed','status'=>'recommended','badge'=>['label'=>'Sabri CF-03','color'=>'blue'],'description'=>'<p>'.esc_html('Decision '.PlatformFinancialPolicy::DECISION_ID.' is active. Complete schema '.CompleteSchema::VERSION.' includes expense and transparency records. Fixed fees are prohibited, donation is optional, and live collection is disabled. '.($status->approved()?'Configured activation evidence is present.':'Missing gates: '.implode(', ',$status->missingGates()).'.')).'</p>','actions'=>'','test'=>'sabri_cf03_activation_gate'];
    }
}
