...
final class WordPressPublicUi
{
    public static function register(): void
    {
        ...
        if (function_exists('wp_register_style') && defined('SABRI_CF03_VERSION')) {
            wp_register_style('sabri-cf03-public', plugins_url('assets/css/public.css', SABRI_CF03_FILE), [], SABRI_CF03_VERSION);
            wp_register_script('sabri-cf03-public', plugins_url('assets/js/public.js', SABRI_CF03_FILE), [], SABRI_CF03_VERSION, true);
        }
    }
    ...
    public static function donate(): string
    {
        ...
        foreach ($copy['amounts'] ?? [] as $amount) {
            if (!is_array($amount) || !isset($amount['minor_units'])) { continue; }
        ...
        $heading = (string)($text['heading'] ?? __('Voluntary One-Time Donation', 'sabri-cf03-finance'));
        ...
        $preparedMessage = $language === 'ur'
            ? 'محفوظ یک وقتی عطیہ وصولی کا نظام ابھی تیاری اور منظوری کے مرحلے میں ہے؛ فی الحال کوئی رقم وصول نہیں کی جارہی۔'
            : 'Secure one-time donation collection is still being prepared and approved; no funds are being collected at this time.';
        ...
        return '<section class="sabri-cf03-card" dir="auto" aria-labelledby="sabri-cf03-donate-title">'
            ...
            .'<input inputmode="decimal" type="text" pattern="[0-9]+([.][0-9]{1,2})?" name="custom_amount" autocomplete="off"'.$disabled.'></label>'
    ...
    public static function transparency(): string
    {
        ...
        if (is_array($snapshot)) {
            ...
            $body .= '<p>'.esc_html__('No verified aggregate financial snapshot has been published. No figures are fabricated.', 'sabri-cf03-finance').'</p>';
        }
    ...
    {
        if (function_exists('wp_enqueue_style')) { wp_enqueue_style('sabri-cf03-public'); }
        ...
        $data = [
            ...
            'messages' => [
                ...
                'consentRequired' => __('Confirm the voluntary one-time donation statement before continuing.', 'sabri-cf03-finance'),
                'secureContext' => __('A secure browser context is required.', 'sabri-cf03-finance'),
                'recorded' => __('Request recorded.', 'sabri-cf03-finance'),
                'unavailable' => __('Secure one-time donation collection is not currently available.', 'sabri-cf03-finance'),
            ],
        ];
        if (function_exists('wp_add_inline_script')) {
            wp_add_inline_script('sabri-cf03-public', 'window.SabriCF03='.wp_json_encode($data).';', 'before');
        }
    }
}
