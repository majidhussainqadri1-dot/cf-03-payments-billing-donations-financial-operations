...
final class WordPressFinancialRepository implements QueryableFinancialRepository
{
    ...
    public function __construct(private readonly object $wpdb, private readonly string $prefix)
    {
        foreach (['prepare','get_row','get_results','get_col','insert','update','delete','query'] as $method) {
            if (!method_exists($wpdb, $method)) { throw new RuntimeException('WordPress database adapter lacks '.$method.'.'); }
        }
        if (preg_match('/^[A-Za-z0-9_]*$/', $prefix) !== 1) { throw new InvalidArgumentException('WordPress database prefix is invalid.'); }
    }
    ...
    public function insert(string $collection, string $id, array $record): void
    {
        ...
        if (isset($record[$idField]) && (string)$record[$idField] !== $id) { throw new InvariantViolation('Financial record identifier mismatch.'); }
        $record[$idField] = $id;
        $data = $this->normalize($table, $record);
        unset($data['id']);
        $result = $this->wpdb->insert($table, $data);
        if ($result !== 1) { throw new InvariantViolation('Financial record insert failed or violated uniqueness.'); }
    }
    ...
    {
        if ($expectedVersion < 1) { throw new InvalidArgumentException('Expected financial record version must be positive.'); }
        [$table,$idField] = $this->spec($collection);
        if (!in_array('record_version', $this->columns($table), true)) { throw new InvariantViolation('Collection does not support compare-and-swap updates.'); }
        $current = $this->get($collection, $id);
        if ($current === null) { throw new InvariantViolation('Financial record not found.'); }
        if ((int)($current['record_version'] ?? 0) !== $expectedVersion) { throw new InvariantViolation('Stale financial record version.'); }
        $next = $mutator($current);
        if (!is_array($next)) { throw new InvariantViolation('Financial record mutator must return an array.'); }
        if (isset($next[$idField]) && (string)$next[$idField] !== $id) { throw new InvariantViolation('Financial record identifier is immutable.'); }
        $next[$idField] = $id;
        $next['record_version'] = $expectedVersion + 1;
        unset($next['version'], $next['id']);
        $data = $this->normalize($table, $next);
        unset($data[$idField]);
        $affected = $this->wpdb->update($table, $data, [$idField=>$id,'record_version'=>$expectedVersion]);
        if ($affected !== 1) { throw new InvariantViolation('Financial compare-and-swap update lost a concurrent race.'); }
        $updated = $this->get($collection, $id);
        if ($updated === null) { throw new InvariantViolation('Updated financial record disappeared.'); }
        return $updated;
    }

    public function all(string $collection): array { return $this->find($collection, [], 500); }

    public function transaction(callable $work): mixed
    {
        $outermost = $this->transactionDepth === 0;
        if ($outermost) { $this->wpdb->query('START TRANSACTION'); }
        $this->transactionDepth++;
        try {
            $result = $work();
            $this->transactionDepth--;
            if ($outermost) { $this->wpdb->query('COMMIT'); }
            return $result;
        } catch (Throwable $error) {
            $this->transactionDepth--;
            if ($outermost) { $this->wpdb->query('ROLLBACK'); }
            throw $error;
        }
    }
    ...
    public function find(string $collection, array $criteria, int $limit = 100): array
    {
        ...
        if ($values !== []) { $sql = $this->wpdb->prepare($sql, ...$values); }
    ...
    {
        if ($criteria === [] || $changes === []) { throw new InvalidArgumentException('Financial update requires bounded criteria and changes.'); }
        [$table] = $this->spec($collection);
        ...
        if ($affected === false) { throw new InvariantViolation('Financial bounded update failed.'); }
        return (int)$affected;
    }
    ...
    public function deleteWhere(string $collection, array $criteria): int
    {
        ...
        if ($affected === false) { throw new InvariantViolation('Financial bounded deletion failed.'); }
        return (int)$affected;
    }
    ...
    private function spec(string $collection): array
    {
        ...
        if ($spec === null) { throw new InvalidArgumentException('Unknown canonical financial collection.'); }
    ...
    private function columns(string $table): array
    {
        ...
        if (!is_array($columns) || $columns === []) { throw new RuntimeException('Canonical financial table is unavailable: '.$table.'.'); }
    ...
    {
        if ($value === null || is_string($value) || is_int($value)) { return $value; }
        ...
        if (is_array($value) && str_ends_with($column, '_json')) {
            ...
            catch (JsonException $error) { throw new InvalidArgumentException('Financial JSON field is not serializable.', 0, $error); }
        }
    ...
    private function hydrate(array $row): array
    {
        foreach ($row as $column => $value) {
            if (is_string($column) && str_ends_with($column, '_json') && is_string($value) && $value !== '') {
                try { $decoded = json_decode($value, true, 512, JSON_THROW_ON_ERROR); if (is_array($decoded)) { $row[$column] = $decoded; } }
                catch (JsonException) { throw new InvariantViolation('Stored financial JSON is corrupt.'); }
            }
        }
        if (isset($row['record_version'])) { $row['record_version'] = (int)$row['record_version']; $row['version'] = $row['record_version']; }
        return $row;
    }
    ...
    {
        if ($criteria === []) { return ['', []]; }
        $columns = $this->columns($table); $clauses = []; $values = [];
        foreach ($criteria as $column => $value) {
            if (!is_string($column) || !in_array($column, $columns, true)) { throw new InvalidArgumentException('Unknown financial query field.'); }
            if ($value === null) { $clauses[] = $column.' IS NULL'; continue; }
            $clauses[] = $column.' = %s'; $values[] = $this->normalizeValue($column, $value);
        }
