...
final class WordPressSchemaInstaller
{
    ...
    public static function install(): array
    {
        ...
        if (! method_exists($wpdb, 'get_var') || ! method_exists($wpdb, 'prepare')) {
            throw new RuntimeException('WordPress database verification methods are unavailable.');
        }
        ...
        if ($missingTables !== []) {
            throw new RuntimeException('CF-03 schema verification failed for: ' . implode(', ', $missingTables) . '.');
        }
        if (count($applied) !== count(Schema::tables((string) $wpdb->prefix))) {
