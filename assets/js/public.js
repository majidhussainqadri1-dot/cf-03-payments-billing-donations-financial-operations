(()=>{
  ...
  document.addEventListener('input',event=>{
    ...
    if(!custom||Number(custom.value)<=0)return;
  ...
  document.addEventListener('submit',async event=>{
    ...
    status.textContent=cfg.messages?.working||'Working…';

    try{
      const data=new FormData(form);
      const selected=data.get('amount_minor');
      const custom=Number(data.get('custom_amount'));
      const minor=custom>0?Math.round(custom*100):Number(selected);
      ...
      status.textContent=result.message||result.status||cfg.messages?.recorded||'Request recorded.';
    }catch(error){
      status.textContent=error instanceof Error?error.message:(cfg.messages?.failed||'Request failed');
      form.elements.idempotency_key.value='';
    }finally{
